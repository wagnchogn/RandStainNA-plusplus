...
from .randstainna import RandStainNA #7.17加入
    ...
    def __call__(self, img):
        ...
        if np.random.rand(1) < self.p:
            ...
            if self.distribution != None:  # 1.30添加，如果有手工指定分布，则全部按照分布的来，否则按照统计的来
                ...
                np_d_true_list = [l_mean['distribution'], l_std['distribution'], a_mean['distribution'],
                                  a_std['distribution'], b_mean['distribution'], b_std['distribution']]
                np_d_sample_list = []
...
def getavgstd(image, seg_np):
    ...
    std.append(image_std_b)
    return (avg, std)
# 7.17引入randstainna_attention
class RandStainNA_Attention(object):
    def __init__(self, fg, bg, color_jitter, randstainna, seg):
        
        self.fg = fg
        self.bg = bg
        #self.cam_name = cam_name

        self.fg_randstainna = RandStainNA(yaml_file=randstainna['fg_yaml_file'], std_hyper=randstainna['std_hyper'],
                                          probability=randstainna['probability'], distribution=randstainna['distribution'],
                                          is_train=True)
        self.fg_cj = HSVJitter(brightness=color_jitter['brightness'], contrast=color_jitter['contrast'],
                             saturation=color_jitter['saturation'], hue=color_jitter['hue'], p=color_jitter['p'])
        self.bg_randstainna = RandStainNA(yaml_file=randstainna['bg_yaml_file'], std_hyper=randstainna['std_hyper'],
                                          probability=randstainna['probability'], distribution=randstainna['distribution'],
                                          is_train=True)
        self.bg_cj = HSVJitter(brightness=color_jitter['brightness'], contrast=color_jitter['contrast'],
                             saturation=color_jitter['saturation'], hue=color_jitter['hue'], p=color_jitter['p'])
        self.seg = seg


    def _offline_location(self, img):
        
        if self.fg == 'randstainna':
            img_fg = self.fg_randstainna(img)
        elif self.fg == 'cj':
            img_fg = self.fg_cj(img)
            
        if self.bg == 'randstainna':
            img_bg = self.bg_randstainna(img)
        elif self.bg == 'cj':
            img_bg = self.bg_cj(img)  # 背景变化小
            
        seg = self.seg
        #seg_v1 = np.asarray(seg)
        #compute how much 1

        seg_np = np.array(seg)[:, :, np.newaxis]/255.0
        avg_fg, std_fg = getavgstd(img_fg, seg_np)
        avg_bg, std_bg = getavgstd(img_bg, (1-seg_np))
        num_ones = np.count_nonzero(seg_np)
        per_fg = num_ones / (seg_np.size)
        img_final = np.array(img_fg) * seg_np + np.array(img_bg) * (1-seg_np)
        img_final = Image.fromarray(np.uint8(img_final))
        
        return img_final,avg_fg,std_fg,avg_bg,std_bg,per_fg
    
    def __call__(self, img):
        # img = resize(img, (32, 32))
        return self._offline_location(img)
    
    def __repr__(self):
        format_string = "("
        format_string += self.fg_randstainna.__repr__()
        format_string += self.bg_cj.__repr__()
        format_string += f")"
        return format_string
    
class FFT_Aug(object):
    
    def __init__ (self, L=0.01, ratio=0.2):
        self.L = L
        self.ratio = ratio
        
    def _low_freq_aug_np(self, amp_img, L=0.01, strength=1):
        # L : 0.01
        # ratio : 0.01左右
        a_img = np.fft.fftshift(amp_img, axes=(-2, -1))
        ...
        a_img[:, h1:h2, w1:w2] *= strength
        a_img = np.fft.ifftshift(a_img, axes=(-2, -1))
        return a_img
        
    def _FFT_SA(self, img, L, strength):
        img = img.convert('RGB')
        ...
        # aug the amplitude part of source with target
        # L: aug area size ratio: aug strength
        amp_img_ = self._low_freq_aug_np(amp_img, L=L, strength=strength)

        # mutated fft of source
        fft_img_ = amp_img_ * np.exp(1j * pha_img)

        # get the auged image
        img_aug = np.fft.ifft2(fft_img_, axes=(-2, -1))
        img_aug = np.real(img_aug)

        # save img
        img_aug = Image.fromarray(img_aug.transpose((1, 2, 0)).clip(0, 255).astype(np.uint8))

        return img_aug

    def __call__(self, img):
        
        L = np.random.uniform(self.L/2.0, self.L*2.0)  # 0.01
        strength = np.random.uniform(1-self.ratio, 1+self.ratio, (3, 1, 1))
        
        return self._FFT_SA(img, L=L, strength=strength)

    # 1.21引入，print内容添加
    def __repr__(self):
        format_string = self.__class__.__name__ + "("
        format_string += f"methods=FFT-Aug"
        format_string += f", L={self.L}"  # 1.30添加，print期望分布
