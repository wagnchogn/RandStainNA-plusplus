...
from torchvision.models import resnet18, resnet50, mobilenet_v3_large, mobilenet_v3_small, efficientnet_b0,convnext_small,shufflenet_v2_x1_0
...
parser.add_argument('--dataset', '-d', metavar='NAME', default='torch/image_folder',
...
parser.add_argument('--test-split', metavar='NAME', default='test',
                    help='dataset test split (default: test)') #12.20添加
...
parser.add_argument('-vb', '--validation-batch-size', type=int, default=128, metavar='N',
...
parser.add_argument('--nj-stdhyper', type=float, default=0, metavar='PCT',
...
parser.add_argument('--nj-p', nargs='+', type=float, default=1,  # 2.9加入，nj的p控制
                    help='norm&jitter possibility (default: [1], range: 0-1)')  # .23改为list，方便random设置
...
parser.add_argument('--fft-aug', nargs='+', type=float, default=None,
                    help='fft-aug L-ratio(default: [0, 0])')

parser.add_argument('--randstainna-attention', nargs='+', type=str, default=None,
                    help='randstainna-attention(default: [0, 0])')
...
parser.add_argument('--seed', type=int, default=97, metavar='S',
                    help='random seed (default: 97)')
...
parser.add_argument('--cam_name',  metavar='NAME', default='cam',
                    help='dataset type (default: ImageFolder/ImageTar if empty)')

parser.add_argument('--ddsgd', action='store_true', default=False,
                    help='dataset type (default: ImageFolder/ImageTar if empty)')
...
def main():
    ...
    if args.local_rank == 0:
        ...
        output_dir = get_outdir(args.output if args.output else './output/train',
                                exp_name + '_' + args.cam_name)  # 1.20修改，相对路径
    print(output_dir)
    ...
    if args.model == 'resnet18':
        ...
        _logger.info('torchvision.convnext_base \n')
    ...
    # 9.24添加randstainna_attention
    if args.randstainna_attention is not None:
        color_jitter = {}
        color_jitter['brightness'] = args.color_jitter[0]
        color_jitter['contrast'] = args.color_jitter[1]
        color_jitter['saturation'] = args.color_jitter[2]
        color_jitter['hue'] = args.color_jitter[3]
        color_jitter['p'] = args.cj_p

        randstainna = {}
        randstainna['bg_yaml_file'] = os.path.join(args.nj_config, args.cam_name, 'bg',
                                                   'CRC_Random(HED+LAB+HSV)_n0.yaml')
        randstainna['fg_yaml_file'] = os.path.join(args.nj_config, args.cam_name, 'fg',
                                                   'CRC_Random(HED+LAB+HSV)_n0.yaml')
        randstainna['std_hyper'] = args.nj_stdhyper
        randstainna['probability'] = args.nj_p
        randstainna['distribution'] = args.nj_distribution

        randstainna_attention = {}
        randstainna_attention['color_jitter'] = color_jitter
        randstainna_attention['randstainna'] = randstainna
        randstainna_attention['fg'] = args.randstainna_attention[0]
        randstainna_attention['bg'] = args.randstainna_attention[1]

    else:
        randstainna_attention = None

    # create the train and eval datasets
    dataset_train = create_dataset(
        args.dataset, root=args.data_dir, split=args.train_split, is_training=True,
        randstainna_attention=randstainna_attention,  # 9.24添加
    ...
    loader_train = create_loader(
        ...
        fft_aug=args.fft_aug,  # 6.20添加
        randstainna_attention_enabled=True if args.randstainna_attention is not None else False,
    ...
    try:
        ...
        Dynamic_P = Dynamic_P_class(epochs=args.epochs, batch_per_epoch=int(len(dataset_train) / args.batch_size) + 1,
                                    dynamic_factor=args.dynamic_factor)  # 2.9新加，传递函数为引用类型即可
        last_best_epoch_test = 0
        best_val_acc = 0
        for epoch in range(start_epoch, args.epochs):
            ...
            train_metrics = train_one_epoch(
                epoch, model, loader_train, optimizer, train_loss_fn, args, nj_dynamic=args.nj_dynamic,
                Dynamic_P=Dynamic_P,
            ...
            if output_dir is not None:
                update_summary(
                    epoch, train_metrics, eval_metrics, test_metrics, os.path.join(output_dir, 'summary.csv'),
            ...
            if saver is not None:
                ...
                # 12.24 增加最好10轮的评估 #2.5增加f1和auc评估
                best_metric, best_metric_val_test, best_epoch, best_metric_test, best_epoch_test, best_f1, best_auc, \
                    best_f1_test, best_auc_test, = saver.save_checkpoint(args.epochs, output_dir, epoch,
                                                                         metric=save_metric,
                                                                         metric_test=save_metric_test,
                                                                         metric_f1=f1_metric,
                                                                         metric_auc=auc_metric)
            if eval_metrics['top1'] > best_val_acc:
                model_dict_val = {
                    'model': model,
                    'epoch': epoch,
                    'acc': best_metric_val_test}
                torch.save(model_dict_val, os.path.join(output_dir, 'val_test_best.pth'))
                best_val_acc = eval_metrics['top1']
            if best_epoch_test != last_best_epoch_test:
                model_dict = {
                    'model': model,
                    'epoch': best_epoch_test,
                    'acc': best_metric_test
                }
                torch.save(model_dict, os.path.join(output_dir, 'best.pth'))
                last_best_epoch_test = best_epoch_test
            # torch.save(model.state_dict(), os.path.join(output_dir, 'current.pth'))
    ...
    model_dict_current = {
        ...
        'epoch': epoch,
        'acc': best_metric_val_test}
    # torch.save(model_dict_current, os.path.join(output_dir, 'current_cam.pth'))
...
def train_one_epoch(
        epoch, model, loader, optimizer, loss_fn, args, nj_dynamic=False, Dynamic_P=None, ddsgd=None,
    ...
    if ddsgd:
        ce_loss_fn = nn.CrossEntropyLoss(reduction="none").cuda()
        kd_loss_fn = DistillationLoss(temp=3.0).cuda()
        #mmd_loss_fn = nn.MSELoss(reduction="none").cuda()
    ...
    if ddsgd == False:
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            input_0, target_0, fg_para_0, bg_para_0, per_fg_0 = batch_1

            input_1, target_1, fg_para_1, bg_para_1, per_fg_1 = batch_2
            input_0, target_0 = input_0.cuda(), target_0.cuda()
            input_1, target_1 = input_1.cuda(), target_1.cuda()
            assert torch.equal(target_0, target_1), "label not mathing"
            fg_para_0, bg_para_0, per_fg_0 = fg_para_0.numpy().copy(), bg_para_0.numpy().copy(), per_fg_0.numpy().copy()
            fg_para_1, bg_para_1, per_fg_1 = fg_para_1.numpy().copy(), bg_para_1.numpy().copy(), per_fg_1.numpy().copy()
            weights = []
            for i in range(fg_para_0.shape[0]):
                fg_consine = 1 - cosine(fg_para_0[i], fg_para_1[i])
                bg_consine = 1 - cosine(bg_para_0[i], bg_para_1[i])
                weight = 1 - (fg_consine * per_fg_0[i] + bg_consine * (1 - per_fg_0[i]))
                weights.append(weight)

            with amp_autocast():

                logit_a, logit_b = model(input_0), model(input_1)
                ce_loss = ce_loss_fn(logit_a, target_0) + ce_loss_fn(
                    logit_b, target_0
                )
                kd_loss = kd_loss_fn(logit_a, logit_b) + kd_loss_fn(
                    logit_b, logit_a
                )
                #mmd_loss = mmd_loss_fn(logit_a, logit_b)

            optimizer.zero_grad()
            kd_loss_1 = stain_Distillation(weights, kd_loss,fg_para_0)
            with amp_autocast():
                loss = (ce_loss + kd_loss_1).mean()

            if not args.distributed:
                losses_m.update(loss.item(), input_0.size(0))
            if loss_scaler is not None:
                loss_scaler(
                    loss, optimizer,
                    clip_grad=args.clip_grad, clip_mode=args.clip_mode,
                ...
                if args.local_rank == 0:
                    _logger.info(
                        ...
                        'LR: {lr:.3e}  '.format(
                            ...
                            loss=losses_m,
                            batch_time=batch_time_m,
                            rate=input_0.size(0) * args.world_size / batch_time_m.val,
                            rate_avg=input_0.size(0) * args.world_size / batch_time_m.avg,
                            lr=lr))
...
def validate(model, loader, loss_fn, args, log_suffix=''):
    ...
    f1, auc = f1_score(target_all, output_all, average='macro'), \
        roc_auc_score(target_all, output_all_auc, average='macro', multi_class='ovr')
    metrics = OrderedDict([('loss', round(losses_m.avg, 4)), ('top1', round(top1_m.avg, 2)),
...
class Dynamic_P_class(object):
    ...
    def __init__(self, epochs=0, batch_per_epoch=0, dynamic_factor=1.0):
        ...
        self.p_list = [math.sin(math.pi * idx / self.batches) * dynamic_factor for idx in range(self.batches)]
        self.i = -1
