...
def ProcessFile(origin_path, save_path, model_cam, cam_extractor, thresh):  # 三个参数，第一个为每个类别的所有图像在计算机中的位置
    ...
    input_tensor = normalize(transforms.ToTensor()(img), [0.5071, 0.4865, 0.4409], [0.2673, 0.2564, 0.2762]).unsqueeze(0)
    
    if j == 0:
        img_batch = input_tensor
        save_path_list.append(save_path)
        j += 1
    elif img_batch.shape[0] < 128:  # 可能存在问题：部分数据没有跑到，最后一轮被忽视了，还是得一张一张跑检查一遍
        ...
        for i in range(cams[0].shape[0]):
            cams_list += [cams[0][i, :, :]]
        segmaps = [to_pil_image((resize(cam.unsqueeze(0), input_tensor.shape[-2:]).squeeze(0) >= thresh).to(dtype=torch.float32)) for cam in cams_list]
...
if __name__ == '__main__':
    ...
    for class_ in file_dirs:  # 嵌套把所有文件夹建好，同时所有路径弄好
        ...
        for img in os.listdir(origin_class_path):
